...
void GestionPoubelles::createMainContent() {
  ...
  poubelleTable->horizontalHeader()->setSectionResizeMode(QHeaderView::Stretch);
  poubelleTable->horizontalHeader()->setSectionResizeMode(
      0, QHeaderView::ResizeToContents);
  poubelleTable->horizontalHeader()->setSectionResizeMode(7,
                                                          QHeaderView::Fixed);

  poubelleTable->setColumnWidth(7, 250); // Give enough space for both buttons
  ...
  poubelleTable->setMinimumHeight(400);
  poubelleTable->setVerticalScrollBarPolicy(Qt::ScrollBarAsNeeded);
  poubelleTable->setHorizontalScrollBarPolicy(Qt::ScrollBarAlwaysOn);
...
void GestionPoubelles::addTableRow(int id, const QString &localisation,
  ...
  connect(modifyBtn, &QPushButton::clicked, this, [this, modifyBtn]() {
    QPoint pos = modifyBtn->parentWidget()->mapTo(poubelleTable, QPoint(0, 0));
    int r = poubelleTable->indexAt(pos).row();
    if (r >= 0)
      onModifyPoubelle(r);
  });
  ...
  connect(deleteBtn, &QPushButton::clicked, this, [this, deleteBtn]() {
    QPoint pos = deleteBtn->parentWidget()->mapTo(poubelleTable, QPoint(0, 0));
    int r = poubelleTable->indexAt(pos).row();
    if (r >= 0)
      onDeletePoubelle(r);
  });
  ...
  poubelleTable->setRowHeight(row, 110);
}
