...
Recyclage::Recyclage(QWidget *parent) : QWidget(parent) {
  ...
  formTitle->setStyleSheet(
      "color:#455A64;font-size:16px;font-weight:600;margin-bottom:10px;");
  ...
  QString inputStyle =
      ...
      "padding:8px; color:#757575; font-weight:600; font-size:13px;"
      "}"
      "QLineEdit:focus, QComboBox:focus { border:1px solid #66BB6A; "
      "background:white; }"
  ...
  table->setStyleSheet(
      "QTableWidget { background:white; border-radius:8px; padding:10px; "
      "border:none; gridline-color:#ECEFF1; font-family: 'Segoe UI', "
      "sans-serif; }"
      "QHeaderView::section { background:#F8F8F8; color:#000000; padding:15px "
      "10px; font-weight:700; font-size: 15px; border-bottom:2px solid "
      "#E0E0E0; border-right: 1px solid #ECEFF1; }"
      "QTableWidget::item { padding:15px 10px; border-bottom:1px solid "
      "#F5F5F5; border-right: 1px solid #ECEFF1; color:#000000; "
  ...
  connect(btnEdit, &QPushButton::clicked, this, [=]() {
    if (idEdit->text().isEmpty() || centreEdit->text().isEmpty()) {
      QMessageBox::warning(
          this, "Erreur",
          "Veuillez remplir les champs obligatoires (ID, Centre).");
      return;
    }
