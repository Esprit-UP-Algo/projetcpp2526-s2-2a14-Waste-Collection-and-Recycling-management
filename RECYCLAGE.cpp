...
Recyclage::Recyclage(QWidget *parent) : QWidget(parent) {
  this->setStyleSheet(
      "QWidget { background-color: #F8F9FB; color: #333; }"
      "QComboBox QAbstractItemView { background-color: white; color: #333; "
      "selection-background-color: #5CB85C; selection-color: white; border: "
      "1px solid #ddd; }");
  ...
  // Note: To avoid replacing the whole file and losing Search fix, I will
  // target the specific sidebar block But replace_file_content needs contiguous
  // block.
  ...
  brandLabel->setStyleSheet("font-size: 24px; font-weight: bold; color: "
  ...
  QLabel *pathLabel =
      new QLabel("Tableau de bord / Tableau de bord / Gestion de recyclage");
  ...
  searchCard->setStyleSheet("background-color: #FFFFFF; border: 1px solid "
                            "#E0E0E0; border-radius: 6px; padding: 15px;");
  ...
  lblSort->setStyleSheet("font-size: 14px; font-weight: bold; color: #000000; "
  ...
  btnPdf->setStyleSheet("QPushButton { background-color: #FF9800; color: "
                        "white; font-weight: bold; padding: 10px 20px; "
                        "border-radius: 4px; border: none; font-size: 14px; }"
                        "QPushButton:hover { background-color: #F57C00; }");

  QPushButton *btnAdd = new QPushButton(" + Ajouter recyclage");
  btnAdd->setStyleSheet("QPushButton { background-color: #A3D977; color: "
                        "white; font-weight: bold; padding: 10px 20px; "
                        "border-radius: 4px; border: none; font-size: "
                        "14px; }"
  ...
  connect(btnPdf, &QPushButton::clicked, this, [=]() {
    ...
    painter.drawText(80, y,
                     "• Quantité totale traitée: " +
                         QString::number(totalQ, 'f', 2) + " kg");
    y += 150;

    // Details Section
    painter.setPen(QColor("#6FA85E"));
    painter.setFont(QFont("Segoe UI", 18, QFont::Bold));
    painter.drawText(50, y, "DÉTAILS PAR TYPE");
    painter.setPen(greenPen);
    painter.drawLine(50, y + 30, width - 50, y + 30);
    y += 100;
    ...
    // Table
    int tableIndent = 50;
    int col1 = tableIndent;
    int col2 = tableIndent + 500;
    int col3 = tableIndent + 900;
    int rowHeight = 80;
    ...
    painter.drawText(col1, y + 55, "Type de Déchet");
    painter.drawText(col2, y + 55, "Quantité (kg)");
    painter.drawText(col3, y + 55, "Pourcentage");
    y += rowHeight;
    ...
    for (auto it = typeStats.begin(); it != typeStats.end(); ++it) {
      ...
      painter.drawText(col1, y + 55, it.key());
      painter.drawText(col2, y + 55, QString::number(it.value(), 'f', 2));
      painter.drawText(col3, y + 55, QString::number(percent, 'f', 1) + "%");
    ...
    painter.drawText(QRect(0, writer.height() - 150, width, 50),
                     Qt::AlignCenter,
                     "TuniWaste - Application de Gestion de Recyclage "
                     "- " +
  ...
  formTitle->setStyleSheet("color:#455A64;font-size:16px;font-weight:600;"
                           "margin-bottom:10px;");
  formCardLayout->addWidget(formTitle);

  QHBoxLayout *formLayout = new QHBoxLayout;

  QString inputStyle = "QLineEdit, QComboBox, QDateEdit, QDoubleSpinBox {"
                       "background:#F5F5F5; border:1px solid #E0E0E0; "
                       "border-radius:4px; "
                       "padding:8px; color:#757575; font-weight:600; "
                       "font-size:13px;"
                       "}"
                       "QLineEdit:focus, QComboBox:focus { border:1px "
                       "solid #66BB6A; "
                       "background:white; }"
  ...
  btnEdit->setStyleSheet("background:#66BB6A;color:white;padding:8px "
                         "20px;border-radius:4px;font-weight:bold;");
  btnDelete->setStyleSheet("background:#EF5350;color:white;padding:8px "
                           "20px;border-radius:4px;font-weight:bold;");
  ...
  table->setHorizontalHeaderLabels(
      {"ID", "Centre", "Responsable", "Type", "Quantité", "Date", "Actions"});
  ...
  table->verticalHeader()->setDefaultSectionSize(
      50); // Increase Row Height for Buttons
  table->setSelectionBehavior(QAbstractItemView::SelectRows);
  table->setEditTriggers(QAbstractItemView::NoEditTriggers);
  table->setShowGrid(false);

  // Table Style match MainWindow
  table->setStyleSheet("QTableWidget { background:white; "
                       "border-radius:8px; padding:10px; "
                       "border:none; gridline-color:#ECEFF1; font-family: "
                       "'Segoe UI', "
                       "sans-serif; }"
                       "QHeaderView::section { background:#F8F8F8; "
                       "color:#000000; padding:15px "
                       "10px; font-weight:700; font-size: 15px; "
                       "border-bottom:2px solid "
                       "#E0E0E0; border-right: 1px solid #ECEFF1; }"
                       "QTableWidget::item { padding:15px 10px; "
                       "border-bottom:1px solid "
                       "#F5F5F5; border-right: 1px solid #ECEFF1; "
                       "color:#000000; "
                       "font-weight:600; font-size:14px; }"
                       "QTableWidget::item:selected { background:#F0F7ED; "
                       "color:#000000; }"
  ...
  connect(btnEdit, &QPushButton::clicked, this, [=]() {
    ...
    if (idEdit->isReadOnly()) {
      ...
      data.push_back({id, centreEdit->text(), typeBox->currentText(),
                      quantiteSpin->value(), dateEdit->date(),
  ...
  connect(btnSort, &QPushButton::clicked, this, [=]() {
    std::sort(data.begin(), data.end(),
              [](const RecyclageItem &a, const RecyclageItem &b) {
  ...
  /* ===== Data Init ===== */
  data.push_back({1, "Centre Tunis", "Plastique", 120.5, QDate(2023, 10, 20),
                  "Ahmed Ben Ali"});
  data.push_back({2, "Centre Ariana", "Verre", 80.0, QDate(2023, 10, 21),
                  "Sami Trabelsi"});
  data.push_back(
      {3, "Centre Sfax", "Papier", 200.0, QDate(2023, 10, 22), "Ali Haddad"});
...
void Recyclage::charger() {
  ...
  QString sCentre = "";
  if (searchCentre)
    sCentre = searchCentre->text().toLower();

  for (int i = 0; i < data.size(); ++i) {
    if (!sId.isEmpty() && !QString::number(data[i].id).contains(sId))
      continue;
    if (!sCentre.isEmpty() && !data[i].centre.toLower().contains(sCentre))
      continue;

    int row = table->rowCount();
    table->insertRow(row);
    table->setColumnWidth(
        6, 180); // Sync width with MainWindow Actions column (180px)
    ...
    table->setItem(row, 4,
                   new QTableWidgetItem(
                       QString::number(data[i].quantite, 'f', 2) + " kg"));
    table->setItem(row, 5,
                   new QTableWidgetItem(data[i].date.toString("dd/MM/yyyy")));
...
void Recyclage::remplirForm(int row) {
  ...
  idEdit->setStyleSheet("background:#E0E0E0; border:1px solid #B0BEC5; "
                        "border-radius:4px; padding:8px; color:#546E7A;");
...
void Recyclage::viderForm() {
  ...
  idEdit->setStyleSheet("background:#F5F5F5; border:1px solid #E0E0E0; "
                        "border-radius:4px; padding:8px; color:#37474F;");
