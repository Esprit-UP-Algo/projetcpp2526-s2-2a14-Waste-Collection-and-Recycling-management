...
GestionZones::GestionZones(QWidget *parent) : QWidget(parent) {
...
void GestionZones::setupUi() {
  ...
  // 1. Sidebar REMOVED (now shared)

  // 2. Main Content
  QWidget *contentWidget = new QWidget();
  contentWidget->setStyleSheet(
      "background-color: #F5F5F5;"); // Ensure light background
  ...
  QString inputStyle =
      "QLineEdit, QComboBox { padding: 8px; border: 1px solid "
      "#ccc; border-radius: 4px; background-color: white; color: #333; } "
      "QComboBox QAbstractItemView { background-color: white; color: #333; "
      "selection-background-color: #9BCB4E; selection-color: white; }";
  ...
  lblId->setStyleSheet("color: #333;");
  formLayout->addWidget(lblId, 0, 0);
  formLayout->addWidget(idEdit, 0, 1);

  QLabel *lblNom = new QLabel("Nom:");
  lblNom->setStyleSheet("color: #333;");
  formLayout->addWidget(lblNom, 0, 2);
  formLayout->addWidget(nomEdit, 0, 3);

  QLabel *lblLoc = new QLabel("Localisation:");
  lblLoc->setStyleSheet("color: #333;");
  formLayout->addWidget(lblLoc, 1, 0);
  formLayout->addWidget(localisationEdit, 1, 1);

  QLabel *lblPop = new QLabel("Population:");
  lblPop->setStyleSheet("color: #333;");
  formLayout->addWidget(lblPop, 1, 2);
  formLayout->addWidget(populationEdit, 1, 3);

  QLabel *lblSurf = new QLabel("Surface:");
  lblSurf->setStyleSheet("color: #333;");
  formLayout->addWidget(lblSurf, 2, 0);
  formLayout->addWidget(surfaceEdit, 2, 1);

  QLabel *lblFreq = new QLabel("Fréquence:");
  lblFreq->setStyleSheet("color: #333;");
  formLayout->addWidget(lblFreq, 2, 2);
  ...
  tableZones->setColumnWidth(6, 220); // Larger width for buttons
  ...
  tableZones->setStyleSheet(
      ...
      "border-radius: 5px; gridline-color: #eee; color: #333; } "
      "QHeaderView::section { "
...
QWidget *GestionZones::createSideBar() {
  ...
  if (!logoPixmap.isNull()) {
    // Programmatically remove white background (cadre blanc)
    ...
    logoPixmap = QPixmap::fromImage(img);

    logo->setPixmap(logoPixmap.scaled(120, 120, Qt::KeepAspectRatio,
                                      Qt::SmoothTransformation));
    logo->setStyleSheet("margin-bottom: 30px; background: transparent;");
  } else {
...
void GestionZones::addTableRow(int row, QString id, QString nom, QString loc,
  ...
  btnEdit->setStyleSheet("background: #5CB85C; color: white; border-radius: "
                         "8px; padding: 5px 10px; font-weight: bold; "
                         "font-size: 14px;"); // Green color
