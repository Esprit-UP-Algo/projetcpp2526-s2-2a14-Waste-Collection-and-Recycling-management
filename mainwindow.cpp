...
void MainWindow::setupUserManagementScreen() {
  ...
  // REMOVED: mainLayout->addWidget(createSidebar());
  ...
  userTable->setHorizontalHeaderLabels(
      {"ID", "Nom", "Email", "Telephone", "Role", "Actions"});
  userTable->horizontalHeader()->setSectionResizeMode(QHeaderView::Stretch);
  userTable->horizontalHeader()->setSectionResizeMode(
      0, QHeaderView::ResizeToContents);
  userTable->horizontalHeader()->setSectionResizeMode(5, QHeaderView::Fixed);
  userTable->horizontalHeader()->resizeSection(
      5, 180); // Fixed width for Actions column
  ...
  userTable->setStyleSheet(
      ...
      "}"
      "QTableWidget::item:selected { background-color: #F0F7ED; color: "
      "#000000; }"
      "QHeaderView::section { "
  ...
  connect(userTable->horizontalHeader(), &QHeaderView::sectionClicked, this,
          &MainWindow::onSortByColumn);
...
void MainWindow::updateUserTable() {
  ...
  for (int i = 0; i < filteredUsers.size(); ++i) {
    ...
    connect(editBtn, &QPushButton::clicked, this, [this, editBtn]() {
      QPoint pos = editBtn->parentWidget()->mapTo(userTable, QPoint(0, 0));
      int r = userTable->indexAt(pos).row();
      if (r >= 0)
        onModifyUser(r);
    });
    ...
    connect(deleteBtn, &QPushButton::clicked, this, [this, deleteBtn]() {
      QPoint pos = deleteBtn->parentWidget()->mapTo(userTable, QPoint(0, 0));
      int r = userTable->indexAt(pos).row();
      if (r >= 0)
        onDeleteUser(r);
    });
...
void MainWindow::onUsersClicked() {
  contentStackedWidget->setCurrentIndex(0); // Show User Management
}
